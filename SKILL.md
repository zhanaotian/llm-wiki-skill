...
**导航 = 覆盖表（整篇课程消化时）**：整篇课程有天然结构（开篇/各专题/收尾），把导航写成覆盖表——**每讲一个专题一条导航：一句完整的话 + 以「归 [[X]]」收尾**（标明该专题知识本体归位于哪一页），条内用「」标关键细节 token（数据点/专名/金句，与将写入主题页的文本一致）。这样 source 导航本身就是"原文段落覆盖表"：导航条数 = 该讲专题块数，每条归位声明 = 该块已归位。收尾跑 `detail-check.sh`（「」token 全库命中）+ `coverage-check.sh`（「」token 在**归位目标页**命中），0 命中即报漏。整篇没有边际片段问题，读完一遍直接进结构提取，覆盖表零额外成本。
