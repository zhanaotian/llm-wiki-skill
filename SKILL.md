...
12b. **文体自检（机器闸门）**：运行 `bash ${SKILL_DIR}/scripts/style-check.sh <vault 根目录>`。`ERROR`（指路尾巴/元叙述，如"见[[X]]、详见、本体见、承载这部分、收在…一节"）必须清零后再进入下一步；`WARN`（`[[wiki/` 前缀链接）逐条确认是合法消歧（`_index` 同名、与素材原文同名）还是需织入化。规则见文末「知识库格式规则（本 fork 定制）」细化 6。
...
3. **生成结构化深度报告**，保存到 `wiki/synthesis/{主题}-{格式}.md`（按 `WIKI_LANG` 切换语言）：
   ...
   **zh**：
   ...
   （综合来源的链接织回正文对应句子，不单列"相关页面"清单——见「知识库格式规则（本 fork 定制）」细化 2/4。）
   ```
   （英文版按「输出语言规则」生成，结构相同。）
   ...
   **模板 C：时间线格式**（触发词：时间线 / 按时间）
   ...
   （来源链接已在"事件说明"里逐条标注，不再单列"相关页面"清单。）
   ```
...
**导航 = 覆盖表（整篇课程消化时，2026-08-10 定稿为 markdown 表格）**：整篇课程有天然结构（开篇/各专题/收尾），`## 知识归位导航` 写成一个三列表：

```
| 专题 | 覆盖要点 | 归位页 |
|------|----------|--------|
| <这块覆盖什么> | <要点，关键细节用「」标（数据点/专名/金句，文本须与将写入目标页的一致，能被 grep -F 命中）> | <[[目标页#锚点]] · [[目标页]]> |
```

每行 = 一个专题；「」token 是 detail-check / coverage-check 的校验钩子；「归位页」列用 `[[X]]` 指向知识本体落点（不用"归/见"尾巴，正文与导航同一套织入式）。收尾跑 `coverage-check.sh`（「」token 在**归位页列目标页**命中）+ `detail-check.sh`（「」token 全库命中），0 命中即报漏（多为 token 文本与原文不一致——对齐原文，或归位页指错——改指实际落点）。整篇没有边际片段问题，读完一遍直接进结构提取，覆盖表零额外成本。
...
3. **独立冷校验**——写完正文后**重新通读原文**（不是看自己的覆盖表）逐段对成稿核一遍。作者对自己刚写的东西是盲的，会脑补出"以为写了其实没写"的细节；冷校验用没参与写作的眼睛重读原文，才看得见被丢的段落。**做法：关键/大篇素材（整讲、易漏长文）派一个独立 subagent**——新上下文、不共享写作记忆，重读原文 + 对照写好的页面报告遗漏；主上下文不背重读成本（省 token），子代理独立上下文没有作者盲区。日常小篇靠 `coverage-check.sh`（归位目标页 token 命中）+ `detail-check.sh`（「」token 全库命中）机械兜底即可。
...
### 细化 6：文体规则的机器闸门（style-check.sh）

文体规则（双链织句、禁指路尾巴）依赖模型自觉时必然漏网——长文生成时注意力在内容上，风格自觉不可靠。把可机器化的规则机器化：`scripts/style-check.sh <vault 根目录>` 扫描 index.md 与 wiki/ 下全部页面（log.md、raw/、素材源目录除外），报告两类问题：

- `ERROR`（退出码 1）：指路尾巴/导航元叙述禁式——`见 [[`、`详见`、`本体见`、`承载这部分`、`收在 [[`、`里接在`、`记录于 [[` 等。**ingest 完成后必须清零**（工作流 2 步骤 12b）；lint（工作流 5）时一并运行。
- `WARN`（不阻断）：`[[wiki/…` 路径前缀链接——存在合法消歧用例（多个 `_index` 同名、裸名与素材原文撞名），逐条人工确认即可；无例外则织入化为裸名。

历史存量（规则定稿前的旧导航句）也以本脚本为准逐步清零；存量改写原则见细化 4：链接必须作为句子成分（主语/宾语/定语），禁"见/详见/本体见/承载/收在…一节"式导航句，允许的织入动词包括"归于、由…展开、承接、消化为"等。
